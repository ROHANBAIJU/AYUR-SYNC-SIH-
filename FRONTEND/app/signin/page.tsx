...
import { demoSignin, demoSignup, storeToken } from "@/lib/api";
...
const SignInPage = () => {
  ...
  const handleSignInSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const resp = await demoSignin(email, password);
      storeToken(resp.access_token);
      // Redirect to dashboard after successful sign-in
      router.push("/dashboard");
    } catch (error: unknown) {
      const msg = error instanceof Error ? error.message : "Sign in failed. If you don't have an account, please sign up.";
      alert(msg);
  ...
  const handleSignUpSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const resp = await demoSignup(name, email, password);
      storeToken(resp.access_token);
      // Redirect to dashboard right after sign-up as well
      router.push("/dashboard");
    } catch (error: unknown) {
